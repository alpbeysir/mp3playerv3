...
public struct SwipeActionData
{
    ...
    public Color color;
    public Action<Track, TrackInfo> onActivate;
}

public class TrackInfo : RecyclingListViewItem, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerClickHandler
{
    ...
    private Track track;
    private Action<Track, TrackInfo> onClick, onLeftSwipe, onRightSwipe, onButton;

    private static TrackInfo swiped;
    ...
    public void Populate(Track t, Action<Track, TrackInfo> _onClick = null)
    {
        ...
        _ = thumbnailDisplay.Set(track.LowResThumbnailUrl);
    ...
    public void SetStatusState(bool state)
    {
        if (state)
        {
            loadingBar.Init();
